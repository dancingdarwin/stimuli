...
% Modified by Jeffrey Wang 3 May 2016 to include 3D case
%     Jeff.B.Wang@stanford.edu
% S_f corrected to be S_f = (u.^2 + v.^2).^(BETA/2);  2/10/05

if numel(DIM) == 2 %2D Case
...
elseif numel(DIM) == 3 %3D case
    ...
    u = repmat(u,1,DIM(2));
    u = repmat(u,1,1,DIM(3));
    
    % v is the set of frequencies along the second dimension.
    v = [(0:floor(DIM(2)/2)) -(ceil(DIM(2)/2)-1:-1:1)]/DIM(2);
    % Reproduce these frequencies along every column
    v = repmat(v,DIM(1),1);
    v = repmat(v,1,1,DIM(3));
    
    % w is the set of frequencies along the third dimension.
    w = [(0:floor(DIM(3)/2)) -(ceil(DIM(3)/2)-1:-1:1)]/DIM(3);
    % Reproduce these frequencies along every column
    w = reshape(w,1,1,[]);
    w = repmat(w,DIM(1),DIM(2),1);
    disp(w(5,5,:))
    % Generate the power spectrum
    S_f = (u.^2 + v.^2).^(BETA);
    S_w = 
    ...
    error('Only spatial and spatiotemporal noise is supported.')
end
